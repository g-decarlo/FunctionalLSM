...
pacman::p_load(
  compositions, data.table, robCompositions, ggplot2, dplyr, maps, mapproj, cowplot, rworldmap, sp, RColorBrewer
)
set.seed(42)
...
# Apply the filter to all relevant data
...
anchor_points_filtered <- find_anchorpoints.lsm(coords_filtered, 12, TRUE)
...
solution_full_functional <- findsolutions.lsm(
  variogram_full, remove_not_convergent = FALSE, lower.delta = 0.5,
...
# Get US map data and filter out Alaska and Hawaii
us_map_full <- map_data("state")
us_map <- filter(us_map_full, !region %in% c("alaska", "hawaii"))

# ---
# 5. PREDICTION AND BACK-TRANSFORMATION
# ---
...
# ---
# 6. GENERATE PLOTS (PUBLICATION READY)
# ---
...
ggsave("Anisotropy_and_Sigma_pub.png", plot_grid(p_aniso, p_sigma, ncol=2), width=14, height=6, dpi=300)
...
# Use the original anchor points for plotting centers
anchor_points_df <- as.data.frame(anchor_points_filtered$anchorpoints)
colnames(anchor_points_df) <- c("lon", "lat")

ellipses_df <- do.call(rbind, lapply(1:nrow(anchor_points_filtered$anchorpoints), function(i) {
  params <- solution_full_functional$solutions[i, ]
  center <- anchor_points_filtered$anchorpoints[i, ]
...
p_ellipses <- ggplot() +
  ...
  coord_map(projection="albers", lat0=39, lat1=45, xlim=c(-125, -67), ylim=c(25,50)) +
  labs(title="Anisotropy Ellipses at Anchor Points", x="Longitude", y="Latitude") + 
  theme_bw(base_size = 14)

ggsave("Anisotropy_Ellipses_pub.png", p_ellipses, width=10, height=7, dpi=300)
...
ggsave("Hazard_Maps_pub.png", plot_grid(p_heavy_cond, p_rain_occur, p_heavy_total, ncol=3), width=18, height=5, dpi=300)
